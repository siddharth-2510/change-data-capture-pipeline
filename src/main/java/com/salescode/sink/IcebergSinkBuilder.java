...
public class IcebergSinkBuilder {
     ...
     * Configured with upsert mode using order_number as the primary key
     */
    public static DataStreamSink<Void> createOrderHeaderSink(
        ...
        return FlinkSink.forRowData(rowStream)
                .tableLoader(tableLoader)
                .writeParallelism(1)
                .equalityFieldColumns(java.util.Arrays.asList("lob", "order_number"))
                .upsert(true)
                .append();
    }
    ...
    // Schema: 60 fields (5 versioning + 54 business + 1 order_details JSON)
    // ============================================================
    private static class OrderHeaderMapper implements MapFunction<ObjectNode, RowData> {
        ...
        public RowData map(ObjectNode node) throws Exception {
            ...
            row.setField(59, toStringData(node, "order_details")); // order_details JSON
